...
void chooseGame(SDL_Window *&window, SDL_Renderer *&rend, TTF_Font *&font)
{
    ...
    SDL_Surface *surface = IMG_Load("Alphabet_image/background.png");
    ...
    surface = TTF_RenderText_Solid(font, "WELCOME TO WORDS", {255, 0, 255});
    text[0] = SDL_CreateTextureFromSurface(rend, surface);
    SDL_FreeSurface(surface);

    surface = TTF_RenderText_Solid(font, "Choose one game", {255, 0, 255});
    text[1] = SDL_CreateTextureFromSurface(rend, surface);
    SDL_FreeSurface(surface);

    surface = TTF_RenderText_Solid(font, "1. Guess the word", {255, 0, 255});
    text[2] = SDL_CreateTextureFromSurface(rend, surface);
    SDL_FreeSurface(surface);

    surface = TTF_RenderText_Solid(font, "2. Search the word", {255, 0, 255});
